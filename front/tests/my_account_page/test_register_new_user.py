...
from backend.src.utilities.genericUtilities import (
...
class TestLoginError:
    ...
    def test_register_new_user_failed(self):
        ...
        actual_error = my_account.wait_until_register_error_pops_up()
        assert expected_error == actual_error, Logger.error(
            f"expected result:{expected_error}, actual result:{actual_error}"
        )
