...
class CredentialsUtility:
    ...
    def get_wc_api_keys(self):
        ...
        if not self.wc_key or not self.wc_secret:
            raise Exception(
                f"The APi Credentials 'WC_KEY' and 'wc_secret' must be in env variable "
    ...
    def get_db_credentials(self):
        ...
        if not self.db_user or not self.db_password:
            raise Exception(
                f"The DB Credentials 'DB_USER' and 'DB_PASSWORD' must be in env variable "
