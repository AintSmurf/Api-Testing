...
class OrdersHelpers:
    ...
    def create_order(self, additional_args=None):
        ...
        if additional_args:
            ...
            ), f"Parameter 'additional args' must be dictionary but found {type(additional_args)}"
            payload.update(additional_args)
        rs_api = self.rs_helper.post(
            "orders", payload=payload, expected_status_code=201
        )
    ...
    def verify_order_is_created(order_json, cs_id, exp_products):
        ...
        assert order_json["customer_id"] == cs_id, (
            "Create order as user expected to have specific ID"
            f"expected id:{cs_id}"
            f"actual id:{order_json['customer_id']}"
        )
        assert len(order_json["line_items"]) == len(
            exp_products
        ), f"Expected only {len(exp_products)} item in order but found {len(order_json['line_items'])}"
        ...
        assert (
            len(line_items) == 1
        ), f"Expected 1 line item but found{len(line_items)}.Order id:{order_id}"
